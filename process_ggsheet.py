...
def update_month_sheet(sheet):
  ...
  sheet.update_values('A3D3', [[
      datetime.now().strftime('%Y-%m'), value_B3, value_C3, value_B3 - value_C3
  ]])
  sheet.sync()


def update_week_sheet(sheet, data):
  weeks = find_sunday_ranges(datetime.now().year, datetime.now().month)
  lenght_of_weeksheet = len(weeks)
  end_row = find_last_row(sheet, 1)
  for i in range(len(weeks)):
    sheet.update_values(f'A{i+7}C{i+7}', [[
        f'{i+1}', weeks[i][0].strftime('%Y-%m-%d'),
        weeks[i][1].strftime('%Y-%m-%d')
    ]])
  for i in range(len(weeks)):
    if compare_String_date(weeks[i][0].strftime('%Y-%m-%d'),
                           weeks[i][1].strftime('%Y-%m-%d'),
                           datetime.now().strftime('%Y-%m-%d')):
      if sheet.get_value(f'D{7+i}') == '': sheet.update_value(f'D{7+i}', 0)
      if sheet.get_value(f'E{7+i}') == '': sheet.update_value(f'E{7+i}', 0)
      sheet.sync()
      sheet.update_value(f'D{7+i}',
                         float(sheet.get_value(f'D{7+i}')) + data[0])
      sheet.update_value(f'E{7+i}',
                         float(sheet.get_value(f'E{7+i}')) + data[1])
  sheet.sync()


def update_daily_sheet_chi(sheet, values):
  first_empty_row = find_last_row(sheet, 1)
  sheet.update_value(f'A{first_empty_row}',
                     datetime.now().strftime('%Y-%m-%d'))
...
def reset_daily_sheet_month(sheet):
  today = datetime.now()
  first_day_of_next_month = (today.replace(day=1) +
                             timedelta(days=32 - today.day)).replace(day=1)
